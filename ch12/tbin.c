...
static struct tbin_node *new_node(struct tbin_tree *tree, int item) {
...
static void print_structure(struct tbin_tree *tree, struct tbin_node *node, int level) {
    char lc[] = "([{<`";
    char rc[] = ")]}>'";
...
/* Examine NODE in a binary tree.  *COUNT is increased by the number
   ...
   error is found. */
static void recurse_tree(struct tbin_node *node, int *count, int parent, int dir, int *okay) {
    assert(count != NULL);
    (*count)++;
...
static void verify_tree(struct tbin_tree *tree, int array[]) {
    ...
    if (okay && tree->root != NULL) {
        ...
        if (okay && count != tree->count) {
            printf(" Tree has %d nodes, but count by right threads is %d.\n", count, tree->count);
            okay = 0;
        }
    }
    ...
    if (okay && tree->root != NULL) {
        ...
        if (okay && count != tree->count) {
            printf(" Tree has %d nodes, but count by left threads is %d.\n", count, tree->count);
            okay = 0;
        }
    }
