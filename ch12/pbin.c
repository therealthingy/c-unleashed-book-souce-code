...
/* Allocates a new node in TREE at *NODE.  Sets the node's parent to
   ...
   appropriately. */
static int new_node(struct pbin_tree *tree, struct pbin_node **node, struct pbin_node *parent, int item) {
...
/* Examine NODE in a binary tree.  *COUNT is increased by the number
   ...
   error is found. */
void recurse_tree(struct pbin_node *node, int *count, int parent, int dir, int *okay) {
    if (node == NULL)
        return;
    ...
    if (parent != INT_MIN) {
        ...
        if (node->parent == NULL) {
            printf(" Node %d has null parent but should have parent %d.\n", node->data, parent);
            *okay = 0;
        } else if (node->parent->data != parent) {
            printf(" Node %d has parent %d but should have parent %d.\n", node->data, node->parent->data, parent);
            *okay = 0;
        }
    } else if (node->parent) {
        printf(" Node %d has parent %d but should have null parent.\n", node->data, node->parent->data);
        *okay = 0;
    }
}
