...
/* Allocates a new node in TREE at *NODE.  Sets the node's parent to
   ...
   appropriately. */
static struct rb_node *new_node(struct rb_tree *tree, int item, enum color color) {
...
void print_structure(struct rb_node *node, int level) {
    ...
    printf(" %c%d%c", "([{`/"[level % 5], node -> data, node -> color == RB_BLACK ? 'r' : 'b');
...
/* Examine NODE in a red-black tree.  *COUNT is increased by the
   ...
   NODE. */
int recurse_tree(struct rb_node *node, int *count, int parent, int dir, int *okay) {
    int lh, rh;
    ...
    if (node->color == RB_RED && node->link[0] && node->link[0]->color == RB_RED) {
        printf(" Red node %d has red left child %d\n", node->data, node->link[0]->data);
        *okay = 0;
    }

    if (node->color == RB_RED && node->link[1] && node->link[1]->color == RB_RED) {
        printf(" Red node %d has red right child %d\n", node->data, node->link[1]->data);
        *okay = 0;
    }
