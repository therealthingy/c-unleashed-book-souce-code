...
int Kruskal_Undirected(struct Graph *G, struct Graph **TreePtr) {
    ...
    */
    ...
    for (i = 0; i < G->NumVertices; i++) {
        ...
        while (!EdgeScanNext(&EScan)) {
            ...
            */
            if (EScan.Source > EScan.Dest)
                SWAP(EScan.Source, EScan.Dest, int);
    ...
    */
        ...
        if (!AreVerticesConnected(Tree, EScan.Source, EScan.Dest)) {
            ...
            if (!j)
                j = ConnectVertex(Tree, EScan.Dest, EScan.Source, EScan.Cost);
            if (j) {
    ...
    for (;;) {
        ...
        */
        ...
        if (W[V].Prev != -1) {
            ...
            if (!i)
                i = ConnectVertex(Tree, W[V].Prev, V, W[V].LowestCost);
            if (i) {
