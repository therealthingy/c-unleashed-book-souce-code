...
{
    if (!G)
        return GRAPH_BADPARAM; /* check for freeing a NULL pointer */
...
*/
{
    ...
    if (!G->Vertices || (*G->Private->AddVertex)(G, Index)) {
        ...
        ptr = realloc(G->Vertices, sizeof(struct Graph_Vertex *) * G->NumVertices);
        if (ptr)
            G->Vertices = ptr; /* if ptr==NULL, then just ignore the error. There's not much else we can do */
...
*/
{
    ...
    if (!G || Index < 0 || Index >= G->NumVertices)
        return GRAPH_BADPARAM;

    for (i = 0; i < G->NumVertices; i++) {
        retval = DisconnectVertex(G, i, Index);
        if (retval)
            return retval; /* should never happen, but just in case */
    }

    (*G->Private->RemoveVertex)(G, Index);

    G->NumVertices--;
    for (i = Index; i < G->NumVertices; i++)
        G->Vertices[i] = G->Vertices[i + 1];

    ptr = realloc(G->Vertices, sizeof(struct Graph_Vertex *) * G->NumVertices);
    if (ptr)
        G->Vertices = ptr; /* ignore the error - it is not critical since we're shrinking, not growing */
...
*/
{
    ...
    */
    if (Ptr) {
        for (i = 0; i < G->NumVertices; i++) {
            if (G->Vertices[i]->Tag.Ptr == Ptr)
                return i;
        }
    } else {
        for (i = 0; i < G->NumVertices; i++) {
            if (G->Vertices[i]->Tag.Num == Num)
                return i;
        }
    }
...
*/
{
    if (Cost == GRAPH_NOTCONNECTED || !G || Source < 0 || Destination < 0 || Source >= G->NumVertices ||
        Destination >= G->NumVertices)
        return GRAPH_BADPARAM;
...
*/
{
    if (!G || Source < 0 || Destination < 0 || Source >= G->NumVertices || Destination >= G->NumVertices)
        return GRAPH_BADPARAM;
...
*/
{
    if (!G || !EScan || Index < 0 || Index >= G->NumVertices)
        return GRAPH_BADPARAM;
