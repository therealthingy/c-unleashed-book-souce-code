...
void AdjMatrix_RemoveVertex(struct Graph *G, int Index) {
    ...
    for (i = 0; i < G->NumVertices; i++) {
        if (i == Index)
            continue;
        for (j = Index; j < G->NumVertices - 1; j++)
            G->Vertices[i]->Edges.Matrix[j] = G->Vertices[i]->Edges.Matrix[j + 1];
...
int AdjMatrix_EdgeScanNext(struct EdgeScan *EScan) {
    ...
    while (EScan->Internal.Index < EScan->G->NumVertices &&
           EScan->G->Vertices[EScan->Source]->Edges.Matrix[EScan->Internal.Index] == GRAPH_NOTCONNECTED)
        EScan->Internal.Index++;
    if (EScan->Internal.Index == EScan->G->NumVertices)
        return 1;
...
struct Graph_Spec AdjMatrix_Spec = {AdjMatrix_MakeGraph,     AdjMatrix_FreeGraph,     AdjMatrix_AddVertex,
                                    AdjMatrix_RemoveVertex,  AdjMatrix_ConnectVertex, AdjMatrix_DisconnectVertex,
                                    AdjMatrix_EdgeScanStart, AdjMatrix_EdgeScanEnd,   AdjMatrix_EdgeScanNext};
