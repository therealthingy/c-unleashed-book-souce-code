...
int GenerateData(FILE *fp, size_t MaxRecs) {
    ...
    char *Name[] = {"Arrow",    "Bizarre",  "Complete",   "Drastic",        "Eagle",          "Fiddleyew",
                    "Gilbert",  "Havago",   "Ingenious",  "J Random Loser", "Kludgit & Runn", "Lightheart",
                    "Mouse",    "Neurosis", "Objective",  "Paradigm",       "Quality",        "Runaway",
                    "Systemic", "Terrible", "Underwater", "Value",          "Wannabee",       "YesWeWill"};

    char *Business[] = {"Advertising", "Building",  "Computers",       "Deliveries", "Engineering",    "Foam Packing",
                        "Garage",      "Hotels",    "Industries",      "Janitorial", "Knitwear",       "Laser Printers",
                        "Mills",       "Notaries",  "Office Cleaning", "Printers",   "Questionnaires", "Radio",
                        "Systems",     "Talismans", "Upholstery",      "Van Hire",   "Waste Disposal", "Yo-yos"};
    ...
    for (ThisRec = 0; ThisRec < MaxRecs; ThisRec++) {
        fprintf(fp, "%c%c%04d/%c%c%c,", Upper[RandInt(26)], Upper[RandInt(26)], RandInt(10000), Lower[RandInt(26)],
                Lower[RandInt(26)], Lower[RandInt(26)]);

        fprintf(fp, "%d/%02d/%02d,", date.tm_year + 1900, /* NB: NOT a Y2K bug! */
                date.tm_mon + 1, date.tm_mday);

        fprintf(fp, "%s ", Name[RandInt(8)]);
        fprintf(fp, "%s Ltd,", Business[RandInt(8)]);
...
int main(int argc, char **argv) {
    ...
    if (argc < 4) {
        ...
        if (fp != NULL) {
            Disclaimer(fp);
            Status = GenerateData(fp, (size_t)strtol(argv[3], NULL, 10));
            fclose(fp);
        }
    }
