    ...
    if (NULL == InFilePtr) {
        ...
        if (NULL == TextBuffer) {
            ...
            if (NULL == HexBuffer) {
                 ...
                 * bulk of the file.
                 */
                while ((BytesRead = fread(TextBuffer, 1, Width, InFilePtr)) > 0) {
                    for (ThisByte = 0; ThisByte < BytesRead; ThisByte++) {
                        /* Insert hex code into hex buffer
                         * at the appropriate offset
                         */
                        sprintf((char *)(HexBuffer + ThisByte * (HexitsPerByte + 1)), "%0*X ", HexitsPerByte,
                    ...
                    printf("%-*.*s | %s\n", HexBuffWidth, HexBuffWidth, HexBuffer, TextBuffer);
                }
                if (ferror(InFilePtr)) {
...
int main(int argc, char **argv) {
    ...
    if (argc < 2 || strcmp("-?", argv[1]) == 0) {
        ...
        if (memcmp(argv[1], "-c", 2) == 0) {
            FilenameArg = 2;
            NumChars = (size_t)strtoul(argv[1] + 2, &EndPointer, 10);
