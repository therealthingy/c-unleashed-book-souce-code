...
static unsigned char *convert(unsigned char *Src, unsigned char *Dest, int Row, int Count) {
    int SourceChar;
...
int main(int argc, char **argv) {
    ...
    for (count = 0; count < LINES_PER_PAGE; ++count) {
        ...
        for (row = 0; row < SCAN_ROWS_PER_CHAR; ++row) {
            convert((unsigned char *)textbuff, binary, row, CHARS_PER_ROW);

            /* write the binary to output file           */
            if ((fwrite(binary, 1, OCTETS_PER_ROW, output)) != OCTETS_PER_ROW) {
