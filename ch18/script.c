...
int main(int argc, char **argv) {
    ...
    if (s_scale) {
        ...
        for (count = 0; count < total; ++count) {
            fprintf(fout, "%f,%f\n", count * x_scale - x_offset, (set[count] * s_scale) - s_offset);
        }
    ...
    fprintf(fout, pline);
    for (count = 0; count < total; ++count) {
        fprintf(fout, "%f,%f\n", count * x_scale - x_offset, (pv[count] * p_scale) - p_offset);
    }
    ...
    if (o_scale) {
        ...
        for (count = 0; count < total; ++count) {
            ...
            d1 *= o_scale;
            d1 -= o_offset;
            fprintf(fout, "%f,%f\n", count * x_scale - x_offset, (out[count] * o_scale) - o_offset);
        }
