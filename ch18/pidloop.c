...
static const PARAM_PARSE parse[] = {
    {'A', &params.acc_ff, -100.0,      100.0, 0, "Acceleration feed forward"},
    {'B',   &params.bias,   0.00,      100.0, 0,             "constant Bias"},
    {'D', &params.d_gain,   0.00,      100.0, 0,           "Derivative gain"},
    {'F', &params.fri_ff,   0.00,      100.0, 0,     "Friction feed forward"},
    {'H',   &params.hold,   0.00,      100.0, 1,               "output Hold"},
    {'I', &params.i_gain,   0.00,      100.0, 0,             "Integral gain"},
    {'L',    &params.lag,   0.00,        1.0, 0,              "response Lag"},
    {'M',    &params.max,      0,      100.0, 0,            "Maximum output"},
    {'N',    &params.min, -100.0,          0, 0,            "miNimum output"},
    {'P', &params.p_gain,   0.00,      100.0, 0,         "Proportional gain"},
    {'R',  &params.accel,   0.00,      100.0, 0,         "acceleration Rate"},
    {'S',  &params.setpt, -10000,      10000, 1,                 "Set point"},
    {'T',  &params.trans,   0.10,        100, 0,            "Transfer ratio"},
    {'V', &params.vel_ff,   0.00,        100, 0,     "Velocity feed forward"},
    {'W',   &params.slew,   0.00,      100.0, 0,                "sleW limit"},
    {'Y', &params.cycles,   0.00, 99999999.0, 0,               "cYcle count"}
};
 ...
 *
 ********************************************************/
enum { ACC_FF, BIAS, D_GAIN, FRI_FF, HOLD, I_GAIN, LAG, MAX, MIN, P_GAIN, ACCEL, SETPT, TRANS, VEL_FF, SLEW, CYCLES };
 ...
 *
 ********************************************************/
static void ShowMenuItem(const PARAM_PARSE *pp) {
    int count;
 ...
 *
 ********************************************************/
static char *SkipWhiteSpace(const char *cp) {
    unsigned int ch;
 ...
 *
 ********************************************************/
static int Compare(const void *ip, const void *pp) {
 ...
 *
 ********************************************************/
static int ParseValue(const PARAM_PARSE *pp, const char *cp, double *d) {
    char *end_ptr;
    double val;
 ...
 *
 ********************************************************/
static PARAM_PARSE *FindParseParams(const char *cp) {
    PARAM_PARSE *pp = NULL;
    int upper;
    ...
    if (cp && strlen(cp) >= 2) {
        ...
        if (isupper(upper) && '=' == cp[1]) {
            ...
            pp = bsearch(&upper, parse, sizeof parse / sizeof *parse, sizeof *parse, Compare);
        }
    }
    return pp;
}
 ...
 *
 ********************************************************/
static int ParseEvent(const char *cp) {
    char *end_ptr;
    PARAM_PARSE *pp;
    EVENT ev = {0};
 ...
 *
 ********************************************************/
static int ParseImmediate(const char *cp) {
    PARAM_PARSE *pp;
 ...
 *
 ********************************************************/
static void ParseParams(const char *fname) {
    FILE *fin;
    char inbuff[1000];
    int upper;
    ...
    for (upper = 0; upper < event_index; ++upper) {
        ...
        if ('Q' == events[upper].event_token) {
            ...
            fprintf(stderr, "%c %f to %p\n", events[upper].event_token, events[upper].event_value,
 ...
 *
 ********************************************************/
static int CheckEvents(unsigned long count) {
    int quit = 0;
    ...
    if (0 != count) {
        ...
        while (0 != events[event_index].event_time && events[event_index].event_time <= count) {
            /* if event token is 'Q', exit the simulation   */
            if ('Q' == events[event_index].event_token) {
                quit = 1;
            } else {
                *events[event_index].event_dest = events[event_index].event_value;
            }
            ++event_index;
        }
    }
...
static int Continue(void) {
    ...
    do {
        ...
        if (isupper(ch)) {
            ...
            pp = bsearch(&ch, parse, sizeof parse / sizeof *parse, sizeof *parse, Compare);
                    ...
                    } else               /* NULL == (pequal = strchr(...))     */
                    {
                        ...
                        if (pp->force_to_int) {
                            fprintf(stderr,
                                    ...
                                    pp->param_name, value, pp->min_val, pp->max_val);
                        } else {
                            fprintf(stderr,
                                    "\nEnter %s current value "
 ...
 *
 ********************************************************/
static double GetCurrentPV(double output) {
 ...
 *
 ********************************************************/
static double ComputePID(double PV) {
    ...
    this_output = params.p_gain * this_error + params.i_gain * integral + params.d_gain * deriv +
                  params.acc_ff * accel + params.vel_ff * next_target + params.fri_ff * friction + params.bias;
...
int main(int argc, char **argv) {
    ...
    for (current_arg = 1; NULL != (args = argv[current_arg]); ++current_arg) {
        if (*args == '-') {
    ...
    while (0 == CheckEvents(repetitions)) {
        ...
        sprintf(results, "%5lu: SP %7.2f PV %7.2f Out %7.2f ", ++repetitions, this_target, PV, current_output);
