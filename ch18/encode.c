...
typedef enum { BLACK_PIXEL = 0x00, WHITE_PIXEL = 0x80, PIXEL_TEST = 0x80 } PIXEL_BITS;

static const char wr_err[] = "encode: error writing %s\n";
static const unsigned char EOL[2] = {0x00, 0x80};
static const unsigned char EOP[9] = {0x00, 0x08, 0x80, 0x00, 0x08, 0x80, 0x00, 0x08, 0x80};
 ...
 */
static unsigned char *OutputCodeWord(const T4_ENCODE *code, unsigned char *t4_ptr) {
    int length;
    ...
    /* otherwise output the code word in the structure        */
        ...
        current_output |= ((unsigned long)code->bit_pattern << length);
 ...
 */
static int CountPixelRun(PIXEL_BITS wanted, int maximum, FILE *fin) {
    int run_count = 0;
    int input;
 ...
 */
int EncodeLine(unsigned char *t4_out, FILE *fin) {
    ...
    while (scan_count > 0) {
        ...
        if (color == BLACK_PIXEL) {
            if (run_count > 63) {
                t4_ptr = OutputCodeWord(black_makeup + (run_count / 64) - 1, t4_ptr);
            }
            t4_ptr = OutputCodeWord(black_terminate + (run_count % 64), t4_ptr);
            color = WHITE_PIXEL;
        } else {
            if (run_count > 63) {
                t4_ptr = OutputCodeWord(white_makeup + (run_count / 64) - 1, t4_ptr);
            }
            t4_ptr = OutputCodeWord(white_terminate + (run_count % 64), t4_ptr);
            color = BLACK_PIXEL;
        }
 ...
 *              page
 */
static int EncodePage(FILE *fin, FILE *fout, unsigned char *buff, char *output_name) {
    int size;
    int scan_lines;
    size_t written;
