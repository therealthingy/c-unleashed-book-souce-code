...
int ReadSyntax(struct sSyntaxTableElement *SyntaxTable, struct sAlternateTableElement *AlternateTable,
               struct sRoutineNameTableElement *RoutineNameTable, char *SyntaxLine, struct sSyntaxHead **pRoot,
    ...
    while (!feof(fSyntaxFile)) {
        iStatus = ProcessSyntaxLine(SyntaxTable, AlternateTable, RoutineNameTable, SyntaxLine, pRootSyntax);
        if (*pRoot == NULL) {
 ...
 */
int FindSyntaxName(struct sSyntaxTableElement *SyntaxTable, char *SyntaxName) {
    int iReturn = 0;
    int i;
    int j;
...
int CreateSyntaxTableEntry(struct sSyntaxTableElement *SyntaxTable, char *pSyntaxName) {
    int iStatus;
...
int GetSyntaxName(struct sSyntaxTableElement *SyntaxTable, char *SyntaxLine, int *j) {
    ...
    if (SyntaxTable[iMySyntaxNumber].pSyntaxPointer == NULL) {
        ...
        if (iMySyntaxNumber > 0) {
             ...
             */
            pNewSyntaxHead->pPreviousHead = SyntaxTable[iMySyntaxNumber - 1].pSyntaxPointer;
            (SyntaxTable[iMySyntaxNumber - 1].pSyntaxPointer)->pNextHead = pNewSyntaxHead;
        } else {
...
int GetLexName(struct sSyntaxTableElement *SyntaxTable, char *SyntaxLine, char *Identifier, int *j,
               int iMySyntaxNumber) {
    int iMyLexNumber = -1;
     ...
     */
    if ((SyntaxLine[*j] == '{') || (SyntaxLine[*j] == '(') || (SyntaxLine[*j] == '[')) {
        /*
...
int GetRoutinePointer(struct sRoutineNameTableElement *RoutineNameTable, char *pszRoutineName,
...
int GetNewSyntaxHead(struct sSyntaxTableElement *SyntaxTable, struct sRoutineNameTableElement *RoutineNameTable,
                     struct sSyntaxHead **ppNewSyntaxHead, char *Identifier) {
    int iStatus = TRUE;
    ...
    *ppNewSyntaxHead = malloc(sizeof(struct sSyntaxHead));
    ...
    if (iNextSyntax > 0) {
         ...
         */
        (*ppNewSyntaxHead)->pPreviousHead = SyntaxTable[iNextSyntax - 1].pSyntaxPointer;
        if ((SyntaxTable[iNextSyntax - 1].pSyntaxPointer) != NULL) {
            (SyntaxTable[iNextSyntax - 1].pSyntaxPointer)->pNextHead = *ppNewSyntaxHead;
        }
    } else {
        /*
    ...
    if (strcmp(Identifier, "") == 0) {
        ...
        GetRoutinePointer(RoutineNameTable, Identifier, &((*ppNewSyntaxHead)->LexRoutine));
 ...
 */
int ProcessSyntaxLine(struct sSyntaxTableElement *SyntaxTable, struct sAlternateTableElement *AlternateTable,
                      struct sRoutineNameTableElement *RoutineNameTable, char *SyntaxLine,
    ...
    if (pC != NULL) {
         ...
         */
        LexIdentifier[0] = '\0';
        GetLexName(SyntaxTable, SyntaxLine, LexIdentifier, &j, iMySyntaxNumber);
        iStatus = GetAlternates(SyntaxTable, AlternateTable, RoutineNameTable, SyntaxLine, &j, pNewSyntaxHead);
 ...
 */
int GetAlternates(struct sSyntaxTableElement *SyntaxTable, struct sAlternateTableElement *AlternateTable,
                  struct sRoutineNameTableElement *RoutineNameTable, char *SyntaxLine, int *j,
                  struct sSyntaxHead *pNewSyntaxHead) {
    int iStatus = TRUE;
    ...
    while ((iStatus == TRUE) && (*j != 0)) {
        ...
        iStatus = GetOneAlternate(SyntaxTable, RoutineNameTable, SyntaxLine, j, pNewAlternate);
 ...
 */
int GetOneAlternate(struct sSyntaxTableElement *SyntaxTable, struct sRoutineNameTableElement *RoutineNameTable,
                    char *SyntaxLine, int *j, struct sSyntaxAlt *pNewAlternate) {
    int iStatus = TRUE;
    int i;
    ...
    while ((i == 0) && (iStatus == TRUE)) {
        ...
        if ((SyntaxLine[*j] == '|') || (SyntaxLine[*j] == '#') || (SyntaxLine[*j] == '\n') ||
            ...
            iStatus = GetSyntaxItem(SyntaxTable, RoutineNameTable, SyntaxLine, j, pNewBody);
            if (!iStatus) {
                i = 1;
                iStatus = TRUE;
            }
...
int GetSyntaxItem(struct sSyntaxTableElement *SyntaxTable, struct sRoutineNameTableElement *RoutineNameTable,
                  char *SyntaxLine, int *j, struct sSyntaxBody *pNewBody) {
    int iStatus = TRUE;
    int i;
    int k;
    int l;
    ...
    case '\"':
        ...
        n = 0;
        for (i = k; (m == 0); i++) {
            if (((SyntaxLine[i] == '\"') && (SyntaxLine[i + 1] != '\\')) || (SyntaxLine[i] == '\n') ||
                (SyntaxLine[i] == '\0')) {
...
void SkipSpaces(char *SyntaxLine, int *j) {
    /*
...
void RemoveSpaces(char *InputBuffer) {
    char *t;
    for (t = InputBuffer; *InputBuffer; !isspace(*InputBuffer) ? *t++ = *InputBuffer++ : *InputBuffer++) {
        continue;
    }
    *t = '\0';
}
...
int paroctal(void *one) {
    ...
    if ((isdigit(*(char *)one)) && (*(char *)one != '8') && (*(char *)one != '9')) {
        iStatus = TRUE;
    } else
        iStatus = FALSE;
...
int parchar(void *one) {
    ...
    if ((*(char *)one == '\'') || (*(char *)one == '\\') || (*(char *)one == '\?') || (*(char *)one == '\"')) {
        iStatus = FALSE;
    } else
        iStatus = TRUE;
...
int BuildNewParseBody(struct sParseNode *pParentHead, struct sParseNode *pPreviousBody, struct sParseNode **ppNewBody,
                      int iFirstChar, int iLastChar) {
    int iStatus;
...
int GetNewParseBody(struct sParseNode *pParentHead, struct sParseNode *pPreviousBody, struct sParseNode **ppNewBody) {
    int iStatus = TRUE;
...
int TryMatchParse(char **ppInputBuffer, int *k, struct sSyntaxBody *pSyntaxP, struct sParseNode **ppParseBody) {
    ...
    if (pSyntaxP != NULL)
        printf("pSyntaxP->BodyContents=:%p:\n", (void *)pSyntaxP->BodyContents);
    if (pSyntaxP->BodyCheck != NULL) {
        ...
        if (((iBodyStringLength + *k) > PARSE_BUFFER_LENGTH) ||
            ((iBodyStringLength + *k) > strlen(pInputBuffer + *k)) || (iBodyStringLength < 1)) {
            /*
        ...
        if (iBodyStringLength == 1)
            printf("%02X|%02X\n", *(pInputBuffer + *k), *pSyntaxP->BodyContents);
        if (strncmp(pInputBuffer + *k, pSyntaxP->BodyContents, iBodyStringLength) != 0) {
        ...
        WAIT *ppInputBuffer += iBodyStringLength;
        ...
        if ((pSyntaxP->BodyHead) != NULL)
            ...
            iStatus = Parse(ppInputBuffer, &kLocal, pSyntaxP->BodyHead, &pNewParse);
        else
            iStatus = TRUE;
        *k = kLocal;
...
int Parse(char **ppInputBuffer, int *k, struct sSyntaxHead *pRootS, struct sParseNode **ppRootP) {
    int iStatus = FALSE;
    ...
    while (bNextExists && iWorkStatus) {
         ...
         *  syntax item:
         */
        iWorkStatus = TryMatchParse(ppInputBuffer, k, pSyntaxP, ppParseBody);
        ...
        *ppRootP = pRootP;
            ...
            if (*ppRootP == NULL) {
                ...
                *ppRootP = NULL;
                ...
                WAIT pRootP = (*ppRootP)->ParentParse;
                free(*ppRootP);
                if (pRootP == NULL) {
                    printf("About to set ppRootP to NULL\n");
                    WAIT
                }
                *ppRootP = pRootP;
            }
...
int ProcessOutputNode(struct sParseNode **ppParseHead, struct sParseNode **ppParseBody) {
    int iStatus = TRUE;
...
void PrintParseTree(struct sParseNode *pInput, int depth) {
    ...
    while (pParseNode != NULL) {
        sp(depth);
        printf("At :%p: points :%p:%p:", (void *)pParseNode, (void *)pParseNode->ThisHead,
        ...
        printf("   :%p:%p:%p:  :%p:%p:%p:\n", (void *)pParseNode->ParentParse, (void *)pParseNode->NextParse,
               (void *)pParseNode->PreviousParse, (void *)pParseNode->ThisParse, (void *)pParseNode->ThisHead,
               (void *)pParseNode->ThisBody);
...
void PrintSyntaxTree(struct sSyntaxTableElement *SyntaxTable, struct sAlternateTableElement *AlternateTable,
                     struct sSyntaxHead *pRootSyntax) {
    ...
    while ((pSyntaxHead != NULL) && (iHead < 20)) {
        iHead++;
        printf("Head name :%s: address  :%p:", pSyntaxHead->SyntaxName, (void *)pSyntaxHead);
#ifdef PARDEBUG
        ...
        while ((pSyntaxAlt != NULL) && (iAlt < 10)) {
            ...
            while ((pSyntaxBody != NULL) && (iBody < 20)) {
                ...
                if (pSyntaxBody->BodyContents != NULL)
                    printf("        == :%p: :%s:\n", (void *)pSyntaxBody->BodyContents, (pSyntaxBody->BodyContents));
                printf("        Syntax Head    :%p:\n", (void *)pSyntaxBody->BodyHead);
                printf("        Syntax Number  :%d: alternate :%d:\n", pSyntaxBody->iSyntaxNumber,
                       pSyntaxBody->iAlternateNumber);
    ...
    printf("---------------TABLES---------------------\n");
    printf("iNextSyntax=:%5.5d: iNextAlternate=:%5.5d:\n", iNextSyntax, iNextAlternate);
    for (i = 0; (i < iNextSyntax); i++) {
        printf("%5.5d :%p: :%s:\n", i, (void *)SyntaxTable[i].pSyntaxPointer, SyntaxTable[i].SyntaxName);
    }
    printf("   ...................................\n");
    for (i = 0; (i < iNextAlternate); i++) {
        printf("%5.5d :%p: :%5.5d:\n", i, (void *)AlternateTable[i].pAlternatePointer, AlternateTable[i].iSyntaxNumber);
    }
    printf("------------------END----------------------\n");
    return;
}
...
#endif
    bGoodSyntax = ReadSyntax(SyntaxTable, AlternateTable, RoutineNameTable, SyntaxLine, &pRootSyntax, argv[1]);
#ifdef PARDEBUGPRINT
    /*#if 0*/
