...
SPARSE_MATRIX *sp_add_header_dimension(SPARSE_MATRIX *sp, int dim, int dim_min, int dim_max)
...
/*    dim_max       The maximum dimension element value */
{
    ...
    for (curr_dim = 0; curr_dim < sp->dimensions; curr_dim++) {
        ...
        if ((new_header->previous != new_header) || (new_header->next != new_header) ||
            (new_header->first != (SP_NODE *)NULL) || (new_header->last != (SP_NODE *)NULL)) {
    ...
    if (dim > sp->dimensions) {
        ...
        (void *)memcpy((void *)new_hdr_stack, (void *)(sp->hdr_stack), sizeof(SP_HDR_ELEMENT) * (sp->dimensions));

        /* Copy the old range stack values into the new stack */
        (void *)memcpy((void *)new_rng_stack, (void *)(sp->hdr_ranges), sizeof(int) * (sp->dimensions) * 2);
