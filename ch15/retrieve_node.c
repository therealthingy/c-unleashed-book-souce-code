...
/* SP_TUPLE *tuple    The tuple used to define the location of the node */
{
    ...
    for (curr_dim = 0; curr_dim < sp->dimensions; curr_dim++) {
        ...
        if ((*curr_seq < sp_get_range_min(sp, curr_dim + 1)) || (sp->error_no != SP_NOERR)) {
            sp->error_no = SP_DLOW;
            return ((SP_NODE *)NULL);
        }
        if ((*curr_seq > sp_get_range_max(sp, curr_dim + 1)) || (sp->error_no != SP_NOERR)) {
    ...
    node_pos = new_list;
    node_found = 1;
    if (memcmp((void *)node_pos->hdr_stack, (void *)new_hdr_list, (sizeof(SP_HDR_ELEMENT *) * sp->dimensions))) {
        do {
            ...
            node_found =
                !memcmp((void *)node_pos->hdr_stack, (void *)new_hdr_list, (sizeof(SP_HDR_ELEMENT *) * sp->dimensions));
