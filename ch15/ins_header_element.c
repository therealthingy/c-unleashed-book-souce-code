...
SPARSE_MATRIX *sp_ins_header_element(SPARSE_MATRIX *sp, int dim, int seq, SP_HEADER_DATA *detail)
                             ...
                             the header */
{
    ...
    } while ((prev_header->sequence < new_header->sequence) && (new_header->sequence < seq));
