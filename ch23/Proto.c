...
char *GetPrototype(char *funcname) {
 ...
 " const char *t, size_t n);</h3>"    },
  ...
  "  int setjmp(jmp_buf env);</h3>"   },
    ...
    qsort(Prototype, count, sizeof Prototype[0], CompProtos);

    Key.Name = funcname;
    p = bsearch(&Key, Prototype, count, sizeof Prototype[0], CompProtos);
...
void App(CGI_LIST *List) {
    ...
    char *VersionInfo = "C Unleashed CGI Demo v1.05";
