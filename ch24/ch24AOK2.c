...
int cascDivide(INT *aNumerator, INT *iDenominator, int places, INT **aAnswer);
int aDivide(INT *aNumerator, INT *aDenominator, int places, INT **aAnswer);
...
int pairMultiply(INT iOne, INT iTwo, INT *pAnswer) {
    ...
    iAnsBot = (iOneBot * iTwoBot) + (iOneBot * iTwoTop * SQRT_BASE) + (iOneTop * iTwoBot * SQRT_BASE);
    ...
    if ((aOne == NULL) || (aTwo == NULL) || (aAnswer == NULL)) {
        ...
        if (j != 0)
            ...
            *(pInt + 1) = aOne[1] + aTwo[1];
             ...
             *              - | -   +
             */
            if (((aOne[0] > 0) && (aTwo[0] < 0)) || ((aOne[0] < 0) && (aTwo[0] > 0)))
                pInt[0] = -pInt[0];
...
int aDivide(INT *aNumerator, INT *aDenominator, int places, INT **aAnswer) {
     ...
     */
    if ((abs(*aNumerator) == 2) && (*(aNumerator + 1) == 0) && (*(aNumerator + 2) == 0)) {
        /*
            ...
            *(plQuotient + j) = aW[1];
            ...
            if (borrow != 0) {
                ...
                *(plQuotient + j) = *(plQuotient + j) - 1;
                ...
                for (i = n + 1; (i > 1); i--) {
                    k = (*plNumerator) - i;
                    *(plNumerator + j + k) += (i == 1) ? 0 : (*(plDenominator + k) + carry);
     ...
     *
     */
    if (((*aNumerator < 0) && (*aDenominator > 0)) || ((*aNumerator > 0) && (*aDenominator < 0)))
        **aAnswer = -**aAnswer;
