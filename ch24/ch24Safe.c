...
double flpSafeDivideSensitive(double flpNumerator, double flpNumeratorPrecision, double flpDenominator,
                              double flpDenominatorPrecision, double *pflpResultPrecision, int RaiseError,
                              int *pErrorRaised) {
    ...
    if (pflpResultPrecision != NULL) {
        ...
        flpLocalResultPrecision =
            (flpLocalNumPrecision * fabs(flpDenominator)) + (flpLocalDenomPrecision * fabs(flpNumerator));
    ...
    /* enough:                                                         */
    flpLocalReturnValue = (long double)flpNumerator / (long double)flpDenominator;
    ...
    if (fabs(flpLocalReturnValue) > DBL_MAX) {
        ...
        if (flpLocalResultPrecision >
            (fabs(flpReturnValue) > DBL_EPSILON ? (fabs(flpReturnValue) * ARITHMETIC_ACCEPT_PRECISION) : DBL_EPSILON)) {
...
double flpSafeDivide(double flpNumerator, double flpDenominator) {
    double flpReturnValue = 0.0;

    flpReturnValue = flpSafeDivideSensitive(flpNumerator, 0.0, flpDenominator, 0.0, NULL, FALSE, NULL);
...
double flpSafeMultiplySensitive(double flpFirstMultiplicand, double flpFirstMultiplicandPrecision,
                                double flpSecondMultiplicand, double flpSecondMultiplicandPrecision,
                                double *pflpResultPrecision, int RaiseError, int *pErrorRaised) {
    double flpReturnValue = 0.0;
    double flpZero = 0.0;
    double flpLocalResultPrecision = 0.0;
    ...
    if (pflpResultPrecision != NULL) {
        ...
        flpLocalResultPrecision = (flpLocalFirstPrecision * fabs(flpSecondMultiplicand)) +
                                  (flpLocalSecondPrecision * fabs(flpFirstMultiplicand));
        if (flpLocalResultPrecision < DBL_EPSILON)
            flpLocalResultPrecision = DBL_EPSILON;
        *pflpResultPrecision = flpLocalResultPrecision;
    }
    ...
    if (flpcmp(flpFirstMultiplicand, "==", 0.0) || flpcmp(flpSecondMultiplicand, "==", 0.0)) {
        if (pflpResultPrecision != NULL) {
            if (flpFirstMultiplicandPrecision > flpSecondMultiplicandPrecision)
                *pflpResultPrecision = flpFirstMultiplicandPrecision;
            else
                *pflpResultPrecision = flpSecondMultiplicandPrecision;
        }
        /* Return to caller with the computed value:                    */
        return (flpReturnValue);
    }
    ...
    if (pflpResultPrecision != NULL) {
        *pflpResultPrecision = ((flpFirstMultiplicandPrecision * fabs(flpSecondMultiplicand)) +
                                (flpSecondMultiplicandPrecision * fabs(flpFirstMultiplicand)));
    }
    ...
    /* ensure that the conversions to (long double) are performed early*/
    /* enough:                                                         */
    flpLocalReturnValue = (long double)flpFirstMultiplicand * (long double)flpSecondMultiplicand;
    /* Now we know the magnitude of the result, adjust the outgoing    */
    /* precision, to account for the minimum error for numbers of this */
    /* value-range:                                                    */
    if (flpLocalResultPrecision < DBL_EPSILON * fabs(flpLocalReturnValue))
...
double flpSafeMultiply(double flpFirstMultiplicand, double flpSecondMultiplicand) {
    double flpReturnValue = 0.0;

    flpReturnValue = flpSafeMultiplySensitive(flpFirstMultiplicand, 0.0, flpSecondMultiplicand, 0.0, NULL, FALSE, NULL);
...
double flpSafeAddSensitive(double flpFirstAddend, double flpFirstAddendPrecision, double flpSecondAddend,
                           double flpSecondAddendPrecision, double *pflpResultPrecision, int RaiseError,
                           int *pErrorRaised) {
    double flpReturnValue = 0.0;
    double flpZero = 0.0;
    double flpLocalResultPrecision = 0.0;
    ...
    if (pflpResultPrecision != NULL) {
        ...
        flpLocalResultPrecision = flpLocalFirstPrecision + flpLocalSecondPrecision;
        if (flpLocalResultPrecision < DBL_EPSILON)
            flpLocalResultPrecision = DBL_EPSILON;
        *pflpResultPrecision = flpLocalResultPrecision;
    }
    ...
    /* ensure that the conversions to (long double) are performed early*/
    /* enough:                                                         */
    flpLocalReturnValue = (long double)flpFirstAddend + (long double)flpSecondAddend;
    /* Now we know the magnitude of the result, adjust the outgoing    */
    /* precision, to account for the minimum error for numbers of this */
    /* value-range:                                                    */
    if (flpLocalResultPrecision < DBL_EPSILON * fabs(flpLocalReturnValue))
...
double flpSafeAdd(double flpFirstAddend, double flpSecondAddend) {
    double flpReturnValue = 0.0;

    flpReturnValue = flpSafeAddSensitive(flpFirstAddend, 0.0, flpSecondAddend, 0.0, NULL, FALSE, NULL);
...
double flpSafeSubtractSensitive(double flpMinuend, double flpMinuendPrecision, double flpSubtrahend,
                                double flpSubtrahendPrecision, double *pflpResultPrecision, int RaiseError,
                                int *pErrorRaised) {
    double flpReturnValue = 0.0;

    /* Since subtraction is simply the reverse of addition, we call the */
    /* Safe Addition function to perform the body:                      */
    flpReturnValue = flpSafeAddSensitive(flpMinuend, flpMinuendPrecision, -flpSubtrahend, flpSubtrahendPrecision,
...
double flpSafeSubtract(double flpMinuend, double flpSubtrahend) {
    double flpReturnValue = 0.0;

    flpReturnValue = flpSafeSubtractSensitive(flpMinuend, 0.0, flpSubtrahend, 0.0, NULL, FALSE, NULL);
