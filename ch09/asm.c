...
static INSTRUCTION InstructionTable[] = {
    "LOADA", ADDRESS,   LOADA, "LOADB",  ADDRESS,   LOADB,  "LOADAI", IMMEDIATE, LOADAI, "LOADBI", IMMEDIATE, LOADBI,
    "NOP",   NONE,      NOP,   "STOREA", ADDRESS,   STOREA, "STOREB", ADDRESS,   STOREB, "ADDA",   IMMEDIATE, ADDA,
    "ADDB",  IMMEDIATE, ADDB,  "COMPAI", IMMEDIATE, COMPAI, "COMPBI", IMMEDIATE, COMPBI, "BLT",    ADDRESS,   BLT,
    "BGT",   ADDRESS,   BGT,   "JUMP",   ADDRESS,   JUMP,   "PRINTA", NONE,      PRINTA, "BREAK",  NONE,      BREAK};
    ...
    while (IsOk && NULL != fgets(Line, MAX_LINE, Fp)) {
        ...
        if (':' == Token[0]) {
            ...
            if (NULL == Instruction) {
                ...
                if (IMMEDIATE == Instruction->Operand || ADDRESS == Instruction->Operand) {
                    /* Get the operand */
...
void Disassemble(int Start, int End, unsigned char *Memory) {
    ...
    for (addr = Start; addr < End; addr++) {
        ...
        if (InsCode < sizeof InstructionTable / sizeof InstructionTable[0]) {
            ...
            case IMMEDIATE:
                addr++;
                printf("%02X      %-8s #%02X\n", Memory[addr], Instruction->InstructionName, Memory[addr]);
                break;

            case ADDRESS:
                addr++;
                printf("%02X %02X   %-8s %04X\n", Memory[addr], Memory[addr + 1], Instruction->InstructionName,
