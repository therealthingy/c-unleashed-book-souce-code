...
int PrintTasks(const void *Object, int Tag, size_t Size, FILE *fp) {
...
int CompareTasks(const void *Left, int LTag, const void *Right, int RTag) {
    const TASK *L = Left;
    const TASK *R = Right;
...
int main(void) {
    ...
    if (NULL != Heap) {
        ...
        for (i = 0; i < NumTasks; i++) {
            HeapInsert(Heap, 0, sizeof TaskList[0], TaskList + i, CompareTasks);
        }
        ...
        while (HeapGetSize(Heap) > 0) {
            HeapDelete(Heap, NULL, NULL, &ThisTask, CompareTasks);
            printf("Time to %s\n", ThisTask.JobName);
        }
