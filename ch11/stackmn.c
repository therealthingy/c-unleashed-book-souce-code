...
int ReadFile(char *Filename, char ***Array, int *NumRows) {
    ...
    *Array = AllocStrArray(LINES_PER_ALLOC, DEFAULT_LINE_LEN);
    if (NULL != *Array) {
        fp = fopen(Filename, "r");
        if (fp != NULL) {
            *NumRows = LINES_PER_ALLOC;
             ...
             */
            while (0 == Error && NULL != fgets(Buffer, sizeof Buffer, fp)) {
...
int Exempt(char *HTMLTag) {
    char *Exceptions[] = {"p", "img", "br", "hr", "dd", "dt", "li"};
...
int main(int argc, char *argv[]) {
    ...
    if (EXIT_SUCCESS == Status) {
        ...
        if (Error != 0) {
            ...
            printf("\nChecking file %s\n\n", Filename);
            for (thisrow = 0; EXIT_SUCCESS == Status && thisrow < rows; thisrow++) {
