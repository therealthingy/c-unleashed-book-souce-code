...
int main(void) {
    ...
    do {
         ...
         */
        if (endp == Buff || 0 == TempCashiers || TempCashiers > (unsigned long)INT_MAX) {
    ...
    for (CurrentSecond = OPENING_TIME; CurrentSecond < CLOSING_TIME; CurrentSecond++) {
        p = RandomP();
        if (p < CHANCE_OF_CUSTOMER) {
             ...
             * him to the queue.
             */
            if (QUEUE_SUCCESS != QueueAdd(&Queue, 0, &Customer, sizeof Customer)) {
        ...
        for (i = 0; i < NumCashiers; i++) {
            ...
            if (Cashier[i].TimeRemaining > 0) {
                ...
                if (QueueCount(&Queue) > 0) {
                    ...
                    Cashier[i].TimeRemaining = (int)(Customer.NumItems * SECONDS_PER_ITEM);
    ...
    if (TotalWaitTime > 0) {
        printf("Average wait time "
               ...
               GetTime((long)(TotalWaitTime / (double)SatisfiedCustomers)));
    }

    printf("\n\nCashier Report\n\n");
    for (i = 0; i < NumCashiers; i++) {
        printf("Cashier %2d saw %d customers ", i, Cashier[i].NumCustomersSeen);
    ...
    printf("Goodwill Penalty  : %12.2f\n", (CustomerNumber - SatisfiedCustomers) * GOODWILL_PENALTY);

    Balance -= (CustomerNumber - SatisfiedCustomers) * GOODWILL_PENALTY;
