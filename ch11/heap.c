...
HEAP *HeapCreate(size_t MaxCount) {
    ...
    if (Heap != NULL) {
        ...
        Heap->Heap = malloc(Heap->MaxCount * sizeof *Heap->Heap);
        if (Heap->Heap == NULL) {
            free(Heap);
            Heap = NULL;
        }
...
int HeapInsert(HEAP *Heap, int Tag, size_t Size, void *Object, HEAP_COMPARE Comp) {
    ...
    if (Okay && Heap->Count >= Heap->MaxCount) {
        Heap->Heap = realloc(Heap->Heap, 2 * Heap->MaxCount * sizeof *Heap->Heap);
        if (Heap->Heap != NULL) {
            Heap->MaxCount *= 2;
        } else {
            Okay = 0;
    ...
    if (Okay) {
        ...
        while (!Done) {
            ...
            /* Step 3. */
            if (i == 0 || (*Comp)(Heap->Heap[i - 1].Object, Heap->Heap[i - 1].Tag, Object, Tag) <= 0) {
 ...
 * least Size bytes.
 */
int HeapDelete(HEAP *Heap, int *pTag, size_t *pSize, void *pObject, HEAP_COMPARE Comp) {
    ...
    if (pObject != NULL) {
        memcpy(pObject, Heap->Heap[0].Object, Heap->Heap[0].Size);
    }
    ...
    while (!Done) {
        ...
        if (j > r) {
            ...
            if (j != r) {
                if ((*Comp)(Heap->Heap[j - 1].Object, Heap->Heap[j - 1].Tag, Heap->Heap[j].Object, Heap->Heap[j].Tag) >
                    0) {
                    j++;
                }
            }
            if ((*Comp)(KeyObject, KeyTag, Heap->Heap[j - 1].Object, Heap->Heap[j - 1].Tag) <= 0) {
                Done = 1;
            } else {
...
int HeapVerify(HEAP *Heap, HEAP_COMPARE Comp, FILE *fp) {
    ...
    for (j = 1; j <= Heap->Count; j++) {
        if (j / 2 >= 1 && (*Comp)(Heap->Heap[j / 2 - 1].Object, Heap->Heap[j / 2 - 1].Tag, Heap->Heap[j - 1].Object,
                                  Heap->Heap[j - 1].Tag) > 0) {
            ++BadCount;
            if (fp != NULL) {
                fprintf(fp, "bad ordering of keys %d and %d\n", j / 2 - 1, j - 1);
            }
        }
    }
...
void HeapDump(const HEAP *Heap, HEAP_PRINT Print, FILE *fp) {
    size_t j;
    ...
    if (fp != NULL) {
        ...
        for (j = 1; j <= Heap->Count; j++) {
            fprintf(fp, "Data %d:\n", j - 1);
            (*Print)(Heap->Heap[j - 1].Object, Heap->Heap[j - 1].Tag, Heap->Heap[j - 1].Size, fp);

            fprintf(fp, "Partner %d:\n", j / 2 - 1);
            if (j / 2 >= 1) {
                (*Print)(Heap->Heap[j / 2 - 1].Object, Heap->Heap[j / 2 - 1].Tag, Heap->Heap[j / 2 - 1].Size, fp);
            } else {
                fprintf(fp, "----\n");
            }
        }
