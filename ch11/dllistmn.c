...
int CalcGreatCircleDistance(CITY *City1, CITY *City2) {
    return (int)(3956.934132687 * acos((sin(City1->Latitude) * sin(City2->Latitude)) +
                                       ((cos(City1->Latitude) * cos(City2->Latitude)) *
                                        ((cos(City1->Longitude) * cos(City2->Longitude)) +
                                         (sin(City1->Longitude) * sin(City2->Longitude))))));
}
...
int main(int argc, char *argv[]) {
    ...
    while (NULL != fgets(Buffer, sizeof Buffer, fp)) {
        if (ParseCity(&ThisCity, Buffer)) {
            if (++NumCities >= MaxCities) {
                ...
                TempCity = realloc(City, MaxCities * sizeof *TempCity);
                if (NULL == TempCity) {
                    free(City);
                    fclose(fp);
                    puts("Out of memory.");
            ...
            memcpy(City + NumCities - 1, &ThisCity, sizeof *City);
        }
    }
    ...
    for (i = 0; i < j; i++) {
        ...
        if (DL_SUCCESS != DLAddAfter(&List, 0, City + k, sizeof *City)) {
    ...
    while (j > 1) {
        ...
        while (DLGetNext(List) != NULL) {
            ...
            Distance = CalcGreatCircleDistance(First, Second);
