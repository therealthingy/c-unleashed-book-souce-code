...
int CUPSInitTCPConnection(CONNECTION *Connection, unsigned long Address, unsigned short Port) {
    int Result = 0;
    ...
    if (!Connection->he) {
        ...
        Connection->their_addr.sin_addr = *((struct in_addr *)Connection->he->h_addr);
        memset(Connection->their_addr.sin_zero, 0, sizeof Connection->their_addr.sin_zero);
    }
...
int CUPSInitUDPConnection(CONNECTION *Connection, unsigned long Address, unsigned short Port) {
    int Result = 0;
...
int CUPSConnect(CONNECTION *Connection) {
    ...
    if (connect(Connection->Socket, (struct sockaddr *)&Connection->their_addr, sizeof(struct sockaddr)) == -1) {
        Result = 1;
    }
    return Result;
}
...
int CUPSCheckForData(CONNECTION *Connection) {
    ...
    if (select(Connection->Socket + 1, &read_fd, NULL, NULL, &tv) == -1)
        return 0;
...
int CUPSBind(CONNECTION *Connection) {
    ...
    if (bind(Connection->Socket, (struct sockaddr *)&Connection->my_addr, sizeof(struct sockaddr)) == -1) {
        Result = 1;
    }
...
int CUPSAcceptConnection(CONNECTION *NewSocket, CONNECTION *ServerConnection) {
    ...
    *NewSocket = *ServerConnection;
    NewSocket->Socket =
        accept(ServerConnection->Socket, (struct sockaddr *)&ServerConnection->their_addr, &ServerConnection->sin_size);
...
int CUPSRecv(CONNECTION *Connection, char *data, size_t size) {
    ...
    if (recv(Connection->Socket, data, (int)size, 0) == -1) {
        Result = 1;
    }
...
int CUPSRecvFrom(CONNECTION *Connection, char *data, size_t size) {
    ...
    if (recvfrom(Connection->Socket, data, size, 0, (struct sockaddr *)&Connection->their_addr,
...
int CUPSSendTo(CONNECTION *Connection, char *data, size_t size) {
    ...
    if (sendto(Connection->Socket, data, size, 0, (struct sockaddr *)&Connection->their_addr,
