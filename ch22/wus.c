...
int main(int argc, char **argv) {
    ...
    while (!done) {
        if (0 != CheckForData(sock)) {
            ...
            if (recvfrom(sock, data, sizeof data, 0, (struct sockaddr *)&their_addr, &address_length) == SOCKET_ERROR) {
            ...
            if (sendto(sock, data, strlen(data), 0, (struct sockaddr *)&their_addr, sizeof(struct sockaddr)) ==
                SOCKET_ERROR) {
