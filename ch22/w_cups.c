...
int CUPSInitTCPConnection(CONNECTION *Connection, unsigned long Address, unsigned short Port) {
    ...
    if (!Connection->he) {
        ...
        Connection->their_addr.sin_addr = *((LPIN_ADDR)*Connection->he->h_addr_list);
...
int CUPSConnect(CONNECTION *Connection) {
    return connect(Connection->Socket, (LPSOCKADDR)&Connection->their_addr, sizeof(struct sockaddr));
}

int CUPSBind(CONNECTION *Connection) {
    int Result = 0;

    if (bind(Connection->Socket, (LPSOCKADDR)&Connection->my_addr, sizeof(struct sockaddr)) == SOCKET_ERROR) {
        Result = 1;
    }
...
int CUPSAcceptConnection(CONNECTION *NewSocket, CONNECTION *ServerConnection) {
    ...
    *NewSocket = *ServerConnection;
    NewSocket->Socket = accept(ServerConnection->Socket, NULL, NULL);
...
int CUPSRecv(CONNECTION *Connection, char *data, size_t size) {
    ...
    if (recv(Connection->Socket, data, (int)size, 0) == SOCKET_ERROR) {
        Result = 1;
    }
...
int CUPSSend(CONNECTION *Connection, char *data, size_t size) {
    ...
    if (send(Connection->Socket, data, (int)size, 0) == SOCKET_ERROR) {
        Result = 1;
    }
