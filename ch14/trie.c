 ...
 */
static trie_result compare_leaf(const struct trie_leaf *leaf, const unsigned char *key, size_t len_key) {
    if (!leaf || len_key != leaf->len_key || 0 != memcmp(key, leaf->key, len_key)) {
        return 0;
    } else
        return leaf->result;
}
 ...
 * any keys
 */
static trie_pointer insert_node(trie_pointer old_node, key_walker *walker, int level, struct trie_leaf *leaf) {
    /*
    ...
    case TRIE_LEAF: {
         ...
         */
        n = extract_at_offset(previous_leaf->key, previous_leaf->len_key, level);
        if (n < 0) {
    ...
    case TRIE_SUBTRIE: {
        ...
        if (n < 0) {
            ...
            if (node->next_level[n]) {
                trie_pointer next_level = insert_node(node->next_level[n], walker, level + 1, leaf);
                if (!next_level) {
                    return 0;
                }
 ...
 */
static int delete_node(trie_pointer *node, key_walker *walker, const unsigned char *key, size_t len_key) {
    ...
    case TRIE_SUBTRIE: {
        ...
        if (n < 0) {
             ...
             */
            assert(len_key == p->exact_match->len_key && 0 == memcmp(key, p->exact_match->key, len_key));
            destroy_leaf(p->exact_match);
            p->exact_match = 0;
        } else {
