...
void *DebugReAllocMemory(void *pOldMem, size_t NewSize, char *FileName, int LineNumber) {
    ...
    if (NULL != NewPtr) {
        ...
        if (NewSize > 0) {
            TrackMemory(MEMTRK_MEMALLOC, 0, NewPtr, NewSize, FileName, LineNumber);
 ...
 */
unsigned long hash1(unsigned long value) { /* N1         P1      K1 */
 ...
 */
unsigned long hash2(unsigned long value) { /* N1         P1       K1 */
...
int MemPrintAllocs(const PAYLOAD *p1, void *p2) {
    ...
    fprintf(fp,
            ...
            p1->Ptr, (unsigned int)p1->Size, p1->Size == 1 ? "e " : "es", p1->LineNumber, p1->FileName);
...
int TrackMemory(MEMTRK_MSG Msg, unsigned long Key, void *Ptr, int Size, char *FileName, int LineNumber) {
    ...
    case MEMTRK_MEMALLOC:
        ...
        if (NULL == AddMemNode(&MemTree, ThisKey, &EntryBuilder)) {
            fprintf(fp, "ERROR in debugging code - "
    ...
    case MEMTRK_MEMFREE:
        ...
        if (NULL != NodePtr) {
            ...
            if (CurrAlloc < 0) {
                fprintf(fp, "ERROR: More memory released "
                            "than allocated!\n");
                fflush(fp);
            }
            ...
            fprintf(fp,
                    ...
                    EntryBuilder.Ptr, EntryBuilder.LineNumber, EntryBuilder.FileName);
            fflush(fp);
        }
    ...
    case MEMTRK_REPORT:
        ...
        if (tmt != NULL) {
            ...
            strftime(timebuffer, sizeof timebuffer, "%H:%M:%S %Z on %A %d %B %Y", tmt);
            fprintf(fp, "\n%s\n\n", timebuffer);
        }

        fprintf(fp, "Current Allocation: %lu byt%s.\n", CurrAlloc, CurrAlloc == 1 ? "e" : "es");

        fprintf(fp, "Maximum Allocation: %lu byt%s.\n", MaxAlloc, MaxAlloc == 1 ? "e" : "es");
...
int WalkMemTree(MEMTREE *Node, int (*Function)(const PAYLOAD *, void *), void *Args) {
    int error = 0;
