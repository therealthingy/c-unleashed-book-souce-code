 ...
 * "c" */
static MATRIX_T *m_muprow(int row, double c, MATRIX_T *a);
...
static void set_low_zero(MATRIX_T *t, int col);

/* swaps rows "row" and "swap" in matrix t */
static void swaprows(MATRIX_T *t, int row, int swap);

/* function to find the row (on or below the diagonal) in
 * which the maximum element of a column occurs */
static int maxelementrow(MATRIX_T *t, int col);
...
static void set_col_zero(MATRIX_T *t, MATRIX_T *v, int col);

/* swap row numbers row and swap in matrices t and v */
static void swaprows2(MATRIX_T *t, MATRIX_T *v, int row, int swap);
...
MATRIX_T *m_add(MATRIX_T *sum, MATRIX_T *a, MATRIX_T *b) {
    int i, j, index;
...
MATRIX_T *m_sub(MATRIX_T *diff, MATRIX_T *a, MATRIX_T *b) {
    int i, j, index;
...
MATRIX_T *m_assign(MATRIX_T *a, MATRIX_T *b) {
    int i, j, index;
    if (a == NULL || b == NULL) {
        mmerrcode = NULLARG;
        return a;
...
MATRIX_T *m_mup(MATRIX_T *prod, MATRIX_T *a, MATRIX_T *b) {
    int i, j, k;
...
MATRIX_T *m_transpose(MATRIX_T *trans, MATRIX_T *a) {
    int i, j;
...
void tprintf(char *comment, char *nformat, MATRIX_T *a, MATRIX_T *b) {
    int i, j, index;
    if (a == NULL || b == NULL) {
        mmerrcode = NULLARG;
...
MATRIX_T *m_assign_identity(MATRIX_T *iden) {
    int i, j, index;
    if (iden == NULL) {
        mmerrcode = NULLARG;
        return iden;
...
double m_getij(MATRIX_T *a, int i, int j) {
    if (a == NULL) {
        mmerrcode = NULLARG;
        return 0.0;
    }
...
int m_putij(MATRIX_T *a, int i, int j, double value) {
    if (a == NULL) {
        mmerrcode = NULLARG;
        return mmerrcode;
    }
...
void m_fputcsv(FILE *fp, MATRIX_T *a) {
    int i, j;
    char *sep;
    char comma[] = ",";
    char nocomma[] = "";
...
void m_seterr(int errcode) {
    mmerrcode = errcode;
}

void m_reseterr(void) {
    mmerrcode = 0;
}
...
MATRIX_T *m_mupconst(MATRIX_T *cprod, double c, MATRIX_T *a) {
    int i, j, index;
...
double m_max_abs_element(MATRIX_T *a) {
    int i, j, index;
...
MATRIX_T *m_inverse(MATRIX_T *v, MATRIX_T *a, double *det, double epsilon) {
...
MATRIX_T *m_muprow(int row, double c, MATRIX_T *a) {
    int j, index;
    if (a == NULL) {
        mmerrcode = NULLARG;
        return a;
...
double m_det(MATRIX_T *a, double epsilon) {
...
MATRIX_T *m_solve(MATRIX_T *x, MATRIX_T *a, MATRIX_T *b, double epsilon) {
...
MATRIX_T *m_ecsolve(MATRIX_T *x, MATRIX_T *a, MATRIX_T *b, double epsilon) {
    /* solves linear equation Ax = b for x using matrix
