...
char const *const ticker[NUM_STOCKS] = {"IBM", "MSFT", "INTC"};

/* File names of the stock data */
char const *const filename[NUM_STOCKS] = {"IBM.DAT", "MSFT.DAT", "INTC.DAT"};
...
void evaluate_fitness(species_t *sp, unsigned int *h, long *fit) {
    int i;
...
int get_recommendation(species_t *sp, unsigned int *h) {
    long total_price;
    long avg_price;
    long price;
    long shares;
    ...
    for (i = 0; i < SPECIES_POPULATION; i++) {
        ...
        for (t = 0; t < HISTORY_LENGTH; t++) {
            ...
            if (-price >= (long)Gray_to_bin(sp->individual[i].buy_price)) {
                /* Buy another share */
                ++shares;
                cash -= (long)h[t];
                last_choice = 1;
            } else if (price >= (long)Gray_to_bin(sp->individual[i].sell_price)) {
                /* Sell another share */
                --shares;
                cash += (long)h[t];
                last_choice = -1;
...
void mate_genes(gene_t *parent_1, gene_t *parent_2, gene_t *child) {
    switch (rand_rng(3)) {
    case 0:
...
void mate_chromosomes(chromosome_t *parent_1, chromosome_t *parent_2, chromosome_t *child) {
    mate_genes(&parent_1->buy_price, &parent_2->buy_price, &child->buy_price);
    mate_genes(&parent_1->sell_price, &parent_2->sell_price, &child->sell_price);
}

void recombine(species_t *s, unsigned int num_survivors) {
    unsigned int i;
    ...
    for (i = num_survivors; i < SPECIES_POPULATION; i++) {
        ...
        mate_chromosomes(&s->individual[parent_1], &s->individual[parent_2], &s->individual[i]);
    }
}
...
void next_generation(population_t *p, history_t *h) {
    ...
    for (s = 0; s < NUM_STOCKS; s++) {
        evaluate_fitness(&p->species[s], h->price[s], fit);

        num_survivors = select_survivors(&p->species[s], &new_species, fit);
...
void migrate(population_t *p) {
    ...
    temp = p->species[NUM_STOCKS - 1].individual[0];
    for (i = 1; i < NUM_STOCKS; i++) {
        p->species[i].individual[0] = p->species[i - 1].individual[0];
    }
...
int main() {
    ...
    for (i = 0; i < NUM_STOCKS; i++) {
        ...
        printf("Recommend %sing %s.\n", (j == 0 ? "hold" : (j == 1 ? "buy" : "sell")), ticker[i]);
    }
